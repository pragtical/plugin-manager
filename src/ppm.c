...
static int ppm_revparse(lua_State* L) {
  git_init();
  git_repository* repository = luaL_checkgitrepo(L, 1);
  git_oid commit_id;
...
static const luaL_Reg system_lib[] = {
  ...
  { "revparse",  ppm_revparse }, // Gets a commit id.
...
#ifdef PPM_STATIC
  extern const char src_ppm_luac[];
  extern unsigned int src_ppm_luac_len;
#endif
...
int main(int argc, char* argv[]) {
  ...
  #else
  if (luaL_loadbuffer(L, src_ppm_luac, src_ppm_luac_len, "ppm.lua") || lua_pcall(L, 0, 1, 0)) {
  #endif
