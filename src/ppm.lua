...
function Pragtical:get_binary_path(arch)
  ...
  return self.local_path .. PATHSEP .. "pragtical" .. get_executable_extension((arch or ARCH)[1])
end
...
function Pragtical:is_installed(arch) return system.stat(self.local_path) ~= nil and self:get_binary_path(arch) end

function Pragtical:reference(reference_type, target, force)
  if reference_type == "share" then
    local share_folder = target .. PATHSEP .. "share" .. PATHSEP .. "pragtical"
    ...
    log.action(string.format("Installing pragtical %s into %s and %s." , self.version, binary_folder, share_folder), "green")
    common.copy(self.datadir_path, share_folder .. PATHSEP .. "data")
    common.copy(self:get_binary_path(ARCH), bin_folder .. PATHSEP .. "pragtical" .. get_executable_extension(ARCH))
  elseif reference_type == "symlink" then
    log.action(string.format("Symlinking pragtical %s to %s." , self.version, target), "green")
    common.symlink(self:get_binary_path(ARCH), target)
  elseif reference_type == "bundle" then
    log.action(string.format("Bundling pragtical %s into %s." , self.version, target), "green")
...
function Pragtical:install(arch)
  if self:is_installed(arch) and not REINSTALL then log.warning("pragtical " .. self.version .. " already installed") return end
  assert(not self:is_local(), "cannot install a local copy of pragtical")
  ...
  if system_bottle.pragtical == self then -- system pragtical. We have to copy it because we can't really set the user directory.
    local executable, datadir = common.path("pragtical" .. get_executable_extension(ARCH[1]))
    ...
    for i,file in ipairs(self.files or {}) do
      ...
      if file_arch and common.grep(ARCH, function(e) return common.grep(file_arch, function(a) return a == e end)[1] end)[1] then
        ...
        if archive then
          ...
          common.rename(self.local_path .. PATHSEP .. "dir" .. PATHSEP .. "pragtical" .. get_executable_extension(file_arch[1]), self.local_path .. PATHSEP .. "pragtical" .. get_executable_extension(file_arch[1]))
  ...
  if not system.stat(self.local_path .. PATHSEP .. "pragtical" .. get_executable_extension(ARCH[1])) then error("can't find executable for pragtical " .. self.version .. "; does this release exist for " .. common.join(" & ", ARCH) .. "?") end
  common.rmrf(local_path)
  common.rename(self.local_path, local_path)
  self.local_path = local_path
end

function Pragtical:uninstall(force)
  ...
  if force or prompt("This will delete " .. self.local_path .. ". Are you sure you want to continue?") then
    log.action("Uninstalling pragtical " .. self.version .. " from "  .. self.local_path)
    common.rmrf(self.local_path)
  end
end
...
function Bottle.new(metadata)
  ...
  if self.name and self:is_constructed() then -- if we exist, and we have a name, find out what pragtical version we are
    local path = self.local_path .. PATHSEP .. "pragtical" .. get_executable_extension(ARCH[1])
...
function Bottle:construct(hardcopy)
  ...
  if self.pragtical and (not self.pragtical:is_installed() or REINSTALL) and not self.pragtical:is_local() then self.pragtical:install() end
  ...
  if pragtical then -- if no pragtical, we're assuming that we're using the system version with a PRAGTICAL_PREFIX environment variable.
    construct(pragtical:get_binary_path(), self.local_path .. PATHSEP .. "pragtical" .. get_executable_extension(ARCH[1]))
    if hardcopy then
      system.chmod(self.local_path .. PATHSEP .. "pragtical" .. get_executable_extension(ARCH[1]), 448) -- chmod to rwx-------\
    end
    construct(pragtical.datadir_path, self.local_path .. PATHSEP .. "data")
    if VERBOSE then log.action(string.format("Constructing bottle from %s, %s", pragtical:get_binary_path(), pragtical.datadir_path)) end
  end
  local installing = {}
...
function Bottle:run(args)
  args = args or {}
  local path = common.exists(self.local_path .. PATHSEP .. "pragtical" .. get_executable_extension(ARCH[1])) or (self.pragtical or primary_pragtical):get_binary_path()
...
local function get_pragticals()
  return common.concat(pragticals, common.flat_map(repositories, function(e) return e.pragticals end))
end
...
function ppm.update(...)
  ...
  for _, pragtical in ipairs(get_pragticals()) do
    if pragtical:is_installed() and not pragtical:is_local() then
      for _, file in ipairs(pragtical.files) do
        if file.checksum == "SKIP" and file.url and #common.intersection(file.arch, ARCH) > 0 then
          local stat = system.stat(pragtical.local_path)
          ...
          if status and err then
            if err['last-modified'] then
              ...
              if lastModified and lastModified > (stat.modified - 120) then
                log.action(string.format("SKIP checksum file in pragtical %s has been changed; %d vs. %d. Redownloading...", pragtical.version, stat.modified, lastModified))
                pragtical:uninstall(true)
                pragtical:install()
              else
...
function ppm.get_pragtical(version)
  ...
  return common.first(get_pragticals(), function(pragtical) return pragtical.version == version end)
end

function ppm.pragtical_save()
  settings.pragticals = common.map(common.grep(pragticals, function(p) return (p:is_local() or not p:is_manifest()) and p.version ~= "system" end), function(p)
    return { version = p.version, mod_version = p.mod_version, path = p.path, binary_path = p.binary_path, datadir_path = p.datadir_path, primary = primary_pragtical == p, files = p.files }
  end)
  ppm.settings_save()
end
...
function ppm.pragtical_add(version, path)
  ...
  if path:find("^http") then
    ...
    local binary_path  = BINARY or (path and(path .. PATHSEP .. "pragtical" .. get_executable_extension(ARCH[1])))
...
function ppm.pragtical_switch(version, target)
  ...
  if DEFAULT_RELEASE_URL and #DEFAULT_RELEASE_URL > 0 then
    if not primary_pragtical:is_installed() then primary_pragtical:install() end
    -- first, we check where the current pragtical is on the path:
    local pragtical_binary = BINARY or common.path("pragtical" .. get_executable_extension(PLATFORM)), "can't find executable"
    local find_new_location = target or not pragtical_binary
    if pragtical_binary then
      local stat = assert(system.stat(pragtical_binary), "can't stat pragtical binary " .. pragtical_binary)
      if SYMLINK ~= false and (stat.symlink or pragtical_binary) == primary_pragtical:get_binary_path() then
        if VERBOSE then log.warning("pragtical " .. primary_pragtical.version .. " is already first on path, making no changes.") end
        return
      end
      -- from there, identify what type of pragtical install this is: in a /share /bin situation, a symlink, or a portable-ish structure.
      local bin_folder = common.dirname(pragtical_binary)
      local usr_folder = common.dirname(bin_folder)
      local share_folder = usr_folder .. PATHSEP .. "share" .. PATHSEP .. "pragtical"
      if not target then
        if stat.symlink then
          -- if the symlinked install corresponds to a pragtical we have on record, then it's likely managed by ppm and can be changed freely.
          if common.first(get_pragticals(), function(l) return l:get_binary_path() == stat.symlink end) then
            common.rmrf(pragtical_binary)
            if SYMLINK == false then
              primary_pragtical:reference("bundle", bin_folder)
            else
              primary_pragtical:reference("symlink", pragtical_binary)
            end
          else
        ...
        elseif system.stat(share_folder) and bin_folder:find("bin$") and not system.stat(bin_folder .. PATHSEP .. "data") then
          -- /bin /share sitatuion
          if REINSTALL then
            -- nuke the binary and /share/pragtical folders
            common.rmrf(pragtical_binary)
            common.rmrf(share_folder)
            if SYMLINK then
              primary_pragtical:reference("symlink", pragtical_binary)
            else
              primary_pragtical:reference("share", usr_folder)
            end
          else
        ...
        elseif system.stat(bin_folder .. PATHSEP .. "data") then
          ...
          if REINSTALL then
            common.rmrf(bin_folder .. PATHSEP .. "pragtical")
            common.rmrf(bin_folder .. PATHSEP .. "data")
            if SYMLINK then
              primary_pragtical:reference("symlink", bin_folder)
            else
              primary_pragtical:reference("bundle", bin_folder)
            end
          else
          ...
          error("unsupported pragtical format (possbily AIO?), please specify a specific switch path")
        end
      end
    end
    if find_new_location then
      -- find a place on the PATH to place a symlink or bundle, or share reference.
      -- it must be BEFORE pragtical_binary in the path
      ...
      local paths
      if target then
        paths = { target, common.dirname(pragtical_binary) }
      else
        paths = common.split(":", os.getenv("PATH"))
        for i, path in ipairs(paths) do
          if pragtical_binary:find(path, 1, true) == 1 then
            binary_index = i
            break
          end
        end
      ...
      for i = binary_index - 1, 1, -1 do
        ...
        local tmp_path = directory .. PATHSEP .. ".ppm-test"
        local f = io.open(tmp_path, "ab")
        if f then f:close() end
        common.rmrf(tmp_path)
        if f then
          if SYMLINK ~= false then
            if prompt(string.format("Install symlink to pragtical %s in %s?", primary_pragtical.version, paths[i])) then
              primary_pragtical:reference("symlink", paths[i] .. PATHSEP .. common.basename(primary_pragtical:get_binary_path()))
            end
          elseif path:find("bin$") and system.stat(common.dirname(paths[i]) .. PATHSEP .. "share") then
            if prompt(string.format("Install pragtical %s into %s and %s?", primary_pragtical.version, paths[i], common.dirname(paths[i]) .. PATHSEP .. "share")) then
              primary_pragtical:reference("share", common.dirname(paths[i]))
            end
          else
            if prompt(string.format("Install pragtical %s into %s as a bundle?", primary_pragtical.version, paths[i])) then
              primary_pragtical:reference("bundle", paths[i])
            end
          end
        end
      end
      assert(i > 0, "Can't find a good place to place a reference to pragtical. Either supply a path, adjust your $PATH or `sudo ppm switch`.")
    end
  end
end
...
function ppm.bottle_dump(name, version, ...)
  local arguments = { ... }
  if not version:find("^%d+") and version ~= "system" and version ~= "primary" then
    table.insert(arguments, 1, version)
    version = nil
  ...
  local addons, i = ppm.retrieve_installable_addons(pragtical, arguments)
  if #arguments >= i then error("invalid use of --") end
  Bottle.new({ pragtical = version and pragtical, name = name, addons = addons, config = CONFIG, local_path = name }):construct(true)
end
...
function ppm.setup()
  ...
  local pragtical_binary = BINARY or common.path("pragtical" .. get_executable_extension(PLATFORM))
  if pragtical_binary then
...
function ppm.command(ARGS)
  ...
  elseif ARGS[2] == "update" then ppm.update(table.unpack(common.slice(ARGS, 3)))
  ...
  elseif ARGS[2] == "pragtical" and ARGS[3] == "upgrade" then return ppm.pragtical_upgrade(table.unpack(common.slice(ARGS, 4)))
  ...
  elseif ARGS[2] == "bottle" and ARGS[3] == "dump" then return ppm.bottle_dump(table.unpack(common.slice(ARGS, 4)))
  ...
  elseif ARGS[2] == "dump" then return ppm.bottle_dump(table.unpack(common.slice(ARGS, 3)))
