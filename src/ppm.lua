...
local HOME, USERDIR, CACHEDIR, JSON, VERBOSE, FILTRATION, MOD_VERSION, QUIET, FORCE, REINSTALL, AUTO_PULL_REMOTES, ARCH, ASSUME_YES, NO_INSTALL_OPTIONAL, TMPDIR, DATADIR, BINARY, POST, PROGRESS, SYMLINK, repositories, pragticals, system_bottle, progress_bar_label, write_progress_bar
...
function Addon:get_install_path(bottle)
  ...
  local path = (((self:is_core(bottle) or self:is_bundled()) and bottle.pragtical.datadir_path) or (bottle.local_path and (bottle.local_path .. PATHSEP .. "user") or USERDIR)) .. PATHSEP .. folder .. PATHSEP .. self.id
...
function Bottle.new(pragtical, addons, is_system)
  local self = setmetatable({
    pragtical = pragtical,
    addons = addons,
    is_system = is_system
  ...
  if not is_system then
    ...
    self.hash = system.hash(pragtical.version .. " " .. common.join(" ", common.map(self.addons, function(p) return p.id .. ":" .. p.version end)))
...
function Bottle:run(args)
  ...
  os.execute(self.local_path .. PATHSEP .. "pragtical" .. (#args > 0 and " " or "") .. table.concat(args, " "))
end
...
local function ppm_repo_init(repos)
  DEFAULT_REPOS = { Repository.url("https://github.com/pragtical/plugin-manager.git:latest") }
...
local function ppm_pragtical_save()
  common.mkdirp(CACHEDIR .. PATHSEP .. "pragticals")
  common.write(CACHEDIR .. PATHSEP .. "pragticals" .. PATHSEP .. "locals.json",
    json.encode(common.map(common.grep(pragticals, function(l) return l:is_local() and not l:is_system() end), function(l) return { version = l.version, mod_version = l.mod_version, path = l.path, binary_path = l.binary_path, datadir_path = l.datadir_path } end))
  )
end
...
local function ppm_pragtical_add(version, path)
  ...
  table.insert(pragticals, Pragtical.new(nil, { version = version, binary_path = { [ARCH[1]] = binary_path }, datadir_path = data_path, path = path:gsub(PATHSEP .. "$", ""), mod_version = MOD_VERSION or LATEST_MOD_VERSION }))
  ppm_pragtical_save()
end
...
local function ppm_pragtical_run(version, ...)
  ...
  local bottle = Bottle.new(pragtical, addons)
...
local function ppm_install(type, ...)
  for i, identifier in ipairs({ ... }) do
    ...
    if id == "pragtical" then
      ...
      local potential_addons = { system_bottle:get_addon(id, version, { mod_version = system_bottle.pragtical.mod_version }) }
...
local function ppm_describe()
  for i,v in ipairs(repositories) do
    if #common.grep(DEFAULT_REPOS, function(r) return r:url() == v:url() end) == 0 then
      io.stdout:write("ppm add " .. v:url() .. " && ")
    end
  end
  print("ppm run " .. system_bottle.pragtical.version .. " " .. common.join(" ", common.map(system_bottle:installed_addons(), function(p) return p.id .. ":" .. p.version end)))
end
...
local function run_command(ARGS)
  ...
  elseif (ARGS[2] == "plugin" or ARGS[2] == "color" or ARGS[2] == "library") and ARGS[3] == "install" then ppm_install(ARGS[2], table.unpack(common.slice(ARGS, 4)))
  elseif (ARGS[2] == "plugin" or ARGS[2] == "color" or ARGS[2] == "library") and ARGS[3] == "uninstall" then ppm_addon_uninstall(ARGS[2], table.unpack(common.slice(ARGS, 4)))
  elseif (ARGS[2] == "plugin" or ARGS[2] == "color" or ARGS[2] == "library") and ARGS[3] == "reinstall" then ppm_addon_reinstall(ARGS[2], table.unpack(common.slice(ARGS, 4)))
  elseif (ARGS[2] == "plugin" or ARGS[2] == "color" or ARGS[2] == "library") and (#ARGS == 2 or ARGS[3] == "list") then return ppm_addon_list(ARGS[2], ARGS[4], ARGS)
...
not commonly used publically.
  ...
  engage_locks(function()
    ...
    if system.stat(CACHEDIR .. PATHSEP .. "pragticals" .. PATHSEP .. "locals.json") then
      for i, pragtical in ipairs(json.decode(common.read(CACHEDIR .. PATHSEP .. "pragticals" .. PATHSEP .. "locals.json"))) do
        table.insert(pragticals, Pragtical.new(nil, { version = pragtical.version, mod_version = pragtical.mod_version, binary_path = pragtical.binary_path, datadir_path = pragtical.datadir_path, path = pragtical.path, tags = { "local" } }))
      end
    end
    ...
    if pragtical_binary then
      ...
      local hash = system.hash(pragtical_binary, "file")
      local system_pragtical = common.first(common.concat(common.flat_map(repositories, function(r) return r.pragticals end), pragticals), function(pragtical) return pragtical.local_path == directory end)
      if not system_pragtical then
        system_pragtical = common.first(pragticals, function(e) return e.version == "system" end)
        if system_pragtical then error("can't find existing system pragtical (does " .. system_pragtical:get_binary_path() .. " exist? was it moved?); run `ppm purge`, or specify --binary and --datadir.") end
        local pragtical_datadirs = { DATADIR, directory:find(PATHSEP .. "bin$") and common.dirname(directory .. PATHSEP .. "share" .. PATHSEP .. "pragtical"), directory .. PATHSEP .. "data" }
        local pragtical_datadir = common.first(pragtical_datadirs, function(p) return p and system.stat(p) end)
        system_pragtical = Pragtical.new(nil, { path = directory, datadir_path = pragtical_datadir, binary_path = { [_G.ARCH] = pragtical_binary }, mod_version = MOD_VERSION or LATEST_MOD_VERSION, version = "system", tags = { "system", "local" } })
      ...
      system_bottle = Bottle.new(system_pragtical, nil, true)
    else
      system_bottle = Bottle.new(Pragtical.new(nil, { mod_version = MOD_VERSION or LATEST_MOD_VERSION, datadir_path = DATADIR, version = "system", tags = { "system", "local" } }), nil, true)
    end
