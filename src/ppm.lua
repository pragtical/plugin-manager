...
local LATEST_MOD_VERSION = "3.3.1"
...
function Bottle.new(pragtical, addons, config, is_system)
  ...
  if not is_system then
    ...
    self.hash = system.hash(pragtical.version .. " " .. common.join(" ", common.map(self.addons, function(p) return (p.repository and p.repository:url() or "") .. ":" .. p.id .. ":" .. p.version end)) .. (config or ""))
...
function Bottle:run(args)
  ...
  local path = self.local_path .. PATHSEP .. "pragtical"
