...
local LATEST_MOD_VERSION = "3.1.0"
local EXECUTABLE_EXTENSION = PLATFORM == "windows" and ".exe" or ""
local HOME, USERDIR, CACHEDIR, JSON, VERBOSE, FILTRATION, MOD_VERSION, QUIET, FORCE, REINSTALL, CONFIG,  NO_COLOR, AUTO_PULL_REMOTES, ARCH, ASSUME_YES, NO_INSTALL_OPTIONAL, TMPDIR, DATADIR, BINARY, POST, PROGRESS, SYMLINK, settings, repositories, pragticals, system_bottle, progress_bar_label, write_progress_bar
...
function Bottle.new(pragtical, addons, config, is_system)
  ...
  if not is_system then
    ...
    self.hash = system.hash(pragtical.version .. " " .. common.join(" ", common.map(self.addons, function(p) return p.id .. ":" .. p.version end)) .. (config or ""))
...
local function run_command(ARGS)
  ...
  elseif ARGS[2] == "unstub" then ppm_unstub(nil, table.unpack(common.slice(ARGS, 3)))
