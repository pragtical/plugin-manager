...
int git_mbedtls_stream_global_init(void)
{
	...
	if (psa_crypto_init() != PSA_SUCCESS) {
		git_error_set(GIT_ERROR_SSL, "failed to initialize PSA crypto");
		goto cleanup;
	}
	...
	/* configure TLSv1.2 or better */
	mbedtls_ssl_conf_min_tls_version(&mbedtls_config, MBEDTLS_SSL_VERSION_TLS1_2);
