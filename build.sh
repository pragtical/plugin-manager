...
# -DPPM_NO_GIT               Compiles without libgit2 support.
# -DPPM_NO_NETWORK           Compiles without network support.
# -DPPM_NO_THREADS           Compiles without threading support.
# -DPPM_NO_REMOTE_EXECUTABLE Compiles without the ability to download remote executables.
# -DPPM_STATIC               Compiles ppm.lua into the binary executable.
# -DPPM_ARCH_TUPLE           Specifies the arch tuple for this build.
# -DPPM_DEFAULT_REPOSITORY   Specifies the default repository to download on init.
# -DPPM_DEFAULT_RELEASE      Specifies the default release for ppm for `self-upgrade`. If set to an empty string, will disable the ability to self-upgrade. This should be done for ppm builds for system package managers.
# -DPPM_VERSION              Specifies the ppm version.
# -DPPM_ARCH_PROCESSOR       Manually specifies the processor archiecture.
# -DPPM_ARCH_PLATFORM        Manually specifies the operating system.
...
[[ "$@" == "clean" ]] && rm -rf lib/libgit2/build lib/zlib/build lib/libzip/build lib/xz/build lib/mbedtls/build lib/prefix lua $BIN *.exe src/ppm.luac src/ppm.lua.c && exit 0
...
if [[ "$@" != *"-lz"* ]]; then
  ...
  [[ ! -e "lib/zlib/build" ]] && { cd lib/zlib && mkdir build && cd build && $CC $COMPILE_FLAGS -DHAVE_FDOPEN -D_LARGEFILE64_SOURCE -I.. ../*.c -c && $AR rc libz.a *.o && cp libz.a ../../prefix/lib && cp ../*.h ../../prefix/include && cd ../../../ || exit -1; }
...
[[ $OSTYPE == 'msys'* || $OSTYPE == 'cygwin'* || $CC == *'mingw'* && "$@" != *"-DPPM_NO_NETWORK"* ]] && LINK_FLAGS="$LINK_FLAGS -lws2_32 -lwinhttp"
[[ $OSTYPE == *'darwin'* ]]                                                  && LINK_FLAGS="$LINK_FLAGS -liconv -framework Foundation"
[[ $OSTYPE == *'darwin'* ]] && "$@" != *"-DPPM_NO_NETWORK"*                  && LINK_FLAGS="$LINK_FLAGS -framework Security"
